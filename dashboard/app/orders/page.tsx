    ...
    <div className="space-y-4 font-[Plus_Jakarta_Sans]">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <span className="text-xs px-2 py-1 rounded-full bg-white/5 border border-white/10 font-medium flex items-center gap-1"><CheckSquare size={12} /> {selected.size} dipilih (multi-select v3.0b)</span>
        ...
        <div className="xy-card rounded-[16px] overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-[13px]">
              <thead className="bg-white/[0.04] text-violet-200/60 text-[11px] uppercase font-semibold tracking-wide">
              ...
              <tbody>
                {orders.slice(0, 50).map((o: any) => (
                  <tr key={o.id} className="border-t border-white/5 hover:bg-white/[0.03]">
                    ...
                    <td className="p-3"><span className="px-2 py-0.5 rounded-full bg-[#7C3AED]/20 border border-[#7C3AED]/30 text-[11px] font-semibold">{o.status}</span></td>
                    <td className="p-3 text-white/50 font-medium">{o.created_at ? new Date(o.created_at).toLocaleDateString("id-ID") : "-"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="xy-card rounded-xl p-3 text-[11px] text-white/30 font-medium">Icons Lucide, font Plus Jakarta Sans, no emoji • multi-select v3.0b</div>
    </div>
  );
}
