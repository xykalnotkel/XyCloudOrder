    ...
    <div className="space-y-4 font-[Plus_Jakarta_Sans]">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-black text-[#1E1B2E] tracking-tight">Pesanan</h1>
        </div>
        <div className="flex gap-2">
          <span className="text-xs px-2 py-1 rounded-full bg-[#F3F0FF] border border-[#E9E3F5] font-medium flex items-center gap-1"><CheckSquare size={12} /> {selected.size} dipilih (multi-select v3.0b)</span>
        ...
        <div className="xy-card rounded-[16px] overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-[13px]">
              <thead className="bg-[#F3F0FF] text-[#7C738F] text-[11px] uppercase font-semibold tracking-wide">
              ...
              <tbody>
                {orders.slice(0, 50).map((o: any) => (
                  <tr key={o.id} className="border-t border-[#E9E3F5] hover:bg-[#F3F0FF]">
                    ...
                    <td className="p-3"><span className="px-2 py-0.5 rounded-full bg-[#F3F0FF] border border-[#E9E3F5] text-[11px] font-semibold">{o.status}</span></td>
                    <td className="p-3 text-[#7C738F] font-medium">{o.created_at ? new Date(o.created_at).toLocaleDateString("id-ID") : "-"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="xy-card rounded-xl p-3 text-[11px] text-[#7C738F] font-medium">Icons Lucide, font Plus Jakarta Sans, no emoji • multi-select v3.0b</div>
    </div>
  );
}
