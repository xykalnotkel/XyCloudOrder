      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-[#9A8CBF] font-medium">Kelola media — endpoint /api/admin/media</p>
          </div>
        </div>
        <span className="text-xs px-3 py-1 rounded-full bg-[#21114A] border border-[#2D1B5E] font-medium">{data.length} item</span>
      </div>
      {loading ? <div className="xy-card rounded-xl p-6 text-center text-[#9A8CBF] font-medium">Memuat...</div> : err ? <div className="xy-card rounded-xl p-4 text-red-300 font-medium">{err} — endpoint /api/admin/media mungkin belum ada, fallback mock.</div> : (
        <div className="xy-card rounded-[16px] overflow-hidden">
          <div className="p-4 text-[12px] text-[#6B5A8A] font-medium">Endpoint: /api/admin/media • {data.length} data dari Worker. Font Plus Jakarta Sans, icons Lucide konsisten.</div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 p-4">
            ...
            {data.length === 0 && (
              <div className="col-span-3 py-12 text-center">
                ...
                <div className="text-[11px] text-[#6B5A8A] mt-1 font-medium">Fetch dari /api/admin/media — pastikan Worker sudah expose route tersebut.</div>
              </div>
            )}
          </div>
        </div>
