      ...
      <Header icon={Terminal} title="Perintah Agen" sub="Antrean perintah ke agen PC (mulai sesi, pasang pin, dll)"
        right={
          <div className="flex gap-1.5 flex-wrap">
            <span className="text-[11px] px-2.5 py-1 rounded-full bg-amber-500/15 border border-amber-500/30 text-amber-700 font-semibold">{n("antre")} antre</span>
            <span className="text-[11px] px-2.5 py-1 rounded-full bg-emerald-500/15 border border-emerald-500/30 text-emerald-700 font-semibold">{n("selesai")} selesai</span>
            <span className="text-[11px] px-2.5 py-1 rounded-full bg-rose-500/10 border border-rose-500/25 text-rose-600 font-semibold">{n("gagal")} gagal</span>
          </div>
        } />
      {loading ? <Load /> : err ? <ErrBox msg={err} /> : (
        <Tabel kosong="Belum ada perintah ke agen."
          kolom={[
            { k: "id", label: "Perintah", render: (r) => <div><div className="font-mono text-[11px] text-[#7C738F]">{r.id}</div><div className="font-semibold text-[#1E1B2E] text-[13px]">{r.jenis}</div></div> },
            { k: "agen", label: "Agen", render: (r) => <div><div className="font-semibold text-[#1E1B2E] text-[12.5px]">{r.agen_nama || r.agen_id}</div>{r.host && <div className="font-mono text-[10.5px] text-[#7C738F]">{r.host}</div>}</div> },
            ...
            { k: "muatan", label: "Muatan", render: (r) =>
              <button onClick={() => setBukaId(bukaId === r.id ? null : r.id)} className="inline-flex items-center gap-1 text-[11px] font-semibold text-[#7C3AED]">
      ...
      {bukaId && (() => {
        ...
        return (
          <div className="xy-card rounded-[16px] p-4">
            <div className="text-[11px] font-semibold text-[#7C738F] uppercase tracking-wide mb-2">Muatan {r.jenis} — {r.id}</div>
