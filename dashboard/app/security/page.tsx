    ...
    <div className="space-y-4 font-[Plus_Jakarta_Sans]">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-violet-200/60 font-medium">Checklist semua lapisan — rate limit, device 2 akun, saldo anti-double, upload validasi, OTP atomik</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="xy-card rounded-[16px] p-5">
          ...
          <ul className="mt-3 space-y-2 text-[13px] text-violet-100/70 font-medium">
        ...
        <div className="xy-card rounded-[16px] p-5">
          ...
          {err ? <div className="mt-3 text-red-300 text-sm font-medium">{err}</div> : !data ? <div className="mt-3 text-white/50 text-sm">Memuat...</div> : (
            <pre className="mt-3 text-[11px] bg-[#100030] p-3 rounded-xl overflow-x-auto text-violet-100/70 font-mono">{JSON.stringify(data, null, 2).slice(0, 4000)}</pre>
          )}
          <div className="mt-4 p-3 rounded-xl bg-[#7C3AED]/10 border border-[#7C3AED]/20 text-[11px] font-medium">File audit lengkap: <code className="font-mono">docs/keamanan-audit.md</code> + <code className="font-mono">docs/rencana-3.3.md</code></div>
        </div>
      </div>
    </div>
  );
