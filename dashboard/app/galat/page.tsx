  ...
  return (
    <div className="space-y-4 font-[Plus_Jakarta_Sans]">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-black text-white tracking-tight">Galat Aplikasi <span className="ml-2 text-[10px] px-2 py-0.5 rounded-full bg-[#21114A] font-bold">BARU</span></h1>
          <p className="text-sm text-[#9A8CBF] font-medium">Crash log dari Android, dikelompokkan by hash, status baru/selesai</p>
        </div>
      </div>
      <div className="xy-card rounded-xl p-4">
        ...
        <div className="space-y-2 max-h-[560px] overflow-auto">
          {list.slice(0,50).map((g:any)=>(
            <div key={g.id} className="py-2.5 px-3 rounded-xl bg-[#21114A] border border-[#2D1B5E] flex items-start justify-between gap-3">
              <div className="flex-1">
                <div className="text-[12px] font-bold text-white">{g.pesan?.slice(0,120)}</div>
                <div className="text-[11px] text-[#6B5A8A] mt-1">{g.perangkat} • {g.versi} • {g.layar} • {g.jumlah}x • {g.terakhir?.slice(0,19)}</div>
              </div>
          ...
          {list.length===0 && <div className="text-[12px] text-[#6B5A8A]">Belum ada laporan — user Android kirim via /api/galat</div>}
        </div>
      </div>
    </div>
  );
