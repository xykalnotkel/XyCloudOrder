      ...
      <Header icon={ArrowLeftRight} title="Riwayat Transaksi" sub="Pergerakan saldo semua pengguna (300 terakhir)"
        right={
          <div className="flex gap-2">
            <span className="text-xs px-3 py-1.5 rounded-full bg-emerald-500/15 border border-emerald-500/30 text-emerald-700 font-semibold">Masuk {rupiah(pemasukan)}</span>
            <span className="text-xs px-3 py-1.5 rounded-full bg-rose-500/10 border border-rose-500/25 text-rose-600 font-semibold">Keluar {rupiah(pengeluaran)}</span>
          </div>
        } />
      ...
      {loading ? <Load /> : err ? <ErrBox msg={err} /> : (
        <Tabel kosong="Belum ada transaksi saldo."
          kolom={[
            ...
            { k: "user", label: "Pengguna", render: (r) => <div><div className="font-semibold text-[#1E1B2E] text-[12.5px]">{r.user_nama || "—"}</div><div className="text-[10.5px] font-mono text-[#7C738F]">{r.user_email || r.user_id}</div></div> },
            ...
            { k: "nominal", label: "Nominal", render: (r) => (
              <span className={`inline-flex items-center gap-1 font-semibold ${Number(r.nominal) > 0 ? "text-emerald-600" : "text-rose-600"}`}>
