  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-black text-white">Banner 🖼️</h1>
          <p className="text-sm text-violet-200/60">Kelola banner home — upload validasi image 5MB, whitelist png/jpeg/webp/gif</p>
        </div>
        <span className="text-xs px-3 py-1 rounded-full bg-white/5 border border-white/10">{data.length} item</span>
      </div>
      {loading ? <div className="xy-card rounded-xl p-6 text-center text-white/60">Memuat...</div> : err ? <div className="xy-card rounded-xl p-4 text-red-300">{err} — endpoint /api/admin/banners mungkin belum ada, fallback mock.</div> : (
        <div className="xy-card rounded-[16px] overflow-hidden">
          <div className="p-4 text-[12px] text-white/40">Endpoint: /api/admin/banners • {data.length} data dari Worker. Jika kosong, UI tetap glossy violet-indigo.</div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 p-4">
            ...
            {data.length === 0 && (
              <div className="col-span-3 py-12 text-center">
                ...
                <div className="text-[11px] text-white/30 mt-1">Fetch dari /api/admin/banners — pastikan Worker sudah expose route tersebut.</div>
              </div>
            )}
          </div>
        </div>
