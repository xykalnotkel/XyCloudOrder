              ...
              {rows.map((r: any, i: number) => (
                <tr key={r.id || i} className="border-b border-[#F0EDFB] last:border-0">
                  <td className="px-4 py-3">
                    <div className="font-semibold">{r.nama_pengundang || r.pengundang || "—"}</div>
                    <div className="text-[11px] text-[#7C738F]">{r.pengundang_id || r.user_id}</div>
                  </td>
                  <td className="px-4 py-3">
                    <div className="font-semibold">{r.nama_diundang || r.diundang || r.nama || "—"}</div>
                    <div className="text-[11px] text-[#7C738F]">{r.diundang_id || r.email}</div>
                  </td>
                  <td className="px-4 py-3">
                    <div className="font-mono text-[11px]">{r.kode || "—"}</div>
                    <div className="font-semibold text-[#7C3AED]">{r.hadiah != null ? rupiah(r.hadiah) : (r.bonus != null ? rupiah(r.bonus) : "—")}</div>
                  </td>
