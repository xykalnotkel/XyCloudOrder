...
export default function UlasanPcPage() {
  ...
  async function muat() {
    ...
    try { setRows(await adminFetch("/api/admin/ulasan-pc")); }
    catch (e: any) { setErr(e.message); }
    finally { setLoading(false); }
  }
  useEffect(() => { muat(); }, []);

  function mulaiBalas(r: any) { setBukaId(r.id); setTeks(r.balasan || ""); }

  async function simpan(id: string) {
    setProses("b_" + id);
    try {
      await adminFetch("/api/admin/ulasan-pc/" + id, { method: "PATCH", body: { balasan: teks.trim() || null } });
  ...
  return (
    <div className="space-y-4 font-[var(--font-inter)]">
      ...
      {loading ? <Load /> : err ? <ErrBox msg={err} /> : rows.length === 0 ? (
        <div className="xy-card rounded-[20px] p-10 text-center text-[#7C738F] font-medium">Belum ada ulasan paket PC.</div>
      ) : (
