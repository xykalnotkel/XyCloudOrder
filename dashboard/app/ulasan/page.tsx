...
export default function UlasanPage() {
  ...
  useEffect(() => {
    adminFetch("/api/admin/ulasan")
      .then((d) => setData(d.data || d.ulasan || d.items || []))
    ...
    <div className="space-y-4 font-[Plus_Jakarta_Sans]">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl xy-btn grid place-items-center"><Star size={18} className="text-white" /></div>
          <div>
            <h1 className="text-xl font-black text-white tracking-tight">Ulasan</h1>
            <p className="text-sm text-violet-200/60 font-medium">Kelola ulasan — endpoint /api/admin/ulasan</p>
          </div>
        </div>
        <span className="text-xs px-3 py-1 rounded-full bg-white/5 border border-white/10 font-medium">{data.length} item</span>
      </div>
      {loading ? <div className="xy-card rounded-xl p-6 text-center text-white/60 font-medium">Memuat...</div> : err ? <div className="xy-card rounded-xl p-4 text-red-300 font-medium">{err} — endpoint /api/admin/ulasan mungkin belum ada, fallback mock.</div> : (
        <div className="xy-card rounded-[16px] overflow-hidden">
          <div className="p-4 text-[12px] text-white/40 font-medium">Endpoint: /api/admin/ulasan • {data.length} data dari Worker. Font Plus Jakarta Sans, icons Lucide konsisten.</div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 p-4">
            ...
            {data.length === 0 && (
              <div className="col-span-3 py-12 text-center">
                ...
                <div className="text-[11px] text-white/30 mt-1 font-medium">Fetch dari /api/admin/ulasan — pastikan Worker sudah expose route tersebut.</div>
              </div>
            )}
          </div>
        </div>
