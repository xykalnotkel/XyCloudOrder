  ...
  return (
    <div className="space-y-4 font-[Plus_Jakarta_Sans]">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-black text-white tracking-tight">Sesi PC <span className="ml-2 text-[10px] px-2 py-0.5 rounded-full bg-[#21114A] font-bold">BARU</span></h1>
          <p className="text-sm text-[#9A8CBF] font-medium">Monitoring sesi aktif, agen, durasi, status siap/berjalan/selesai</p>
        </div>
      </div>
      ...
      <div className="xy-card rounded-xl p-4">
        <div className="space-y-2 max-h-[560px] overflow-auto">
          {sesi.slice(0,50).map((s:any)=>(
            <div key={s.id} className="flex items-center justify-between py-2 border-b border-[#2D1B5E] text-[12px]">
              <div><div className="font-bold text-white">{s.id} • {s.nama} ({s.email})</div><div className="text-[#6B5A8A] text-[11px]">{s.unit} • {s.status} • {s.dibuat?.slice(0,19)}</div></div>
              <span className="text-[10px] px-2 py-1 rounded-full bg-[#21114A] text-[#A78BFA] font-bold">{s.status}</span>
            </div>
          ))}
          {sesi.length===0 && <div className="text-[12px] text-[#6B5A8A]">Belum ada sesi — endpoint /api/admin/sesi</div>}
        </div>
      </div>
    </div>
  );
