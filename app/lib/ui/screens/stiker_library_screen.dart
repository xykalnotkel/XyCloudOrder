...
class _StikerLibraryScreenState extends State<StikerLibraryScreen>{
  ...
  @override Widget build(BuildContext context){final pal=XyTheme.of(context);return Scaffold(appBar:AppBar(title:const Text('Stiker & Penyimpanan'),actions:[IconButton(tooltip:'Tambah stiker',onPressed:add,icon:const Icon(Icons.add_rounded))]),
    body:RefreshIndicator(onRefresh:load,child:ListView(padding:const EdgeInsets.all(20),children:[
      XyCard(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
        const Text('Tersimpan di aplikasi ini',style:TextStyle(fontSize:19,fontWeight:FontWeight.w800)),const SizedBox(height:8),
      ...
      const SizedBox(height:22),Row(children:[const Expanded(child:Text('Koleksi kamu',style:TextStyle(fontWeight:FontWeight.w800,fontSize:17))),TextButton(onPressed:add,child:const Text('Tambah'))]),
  ...
  Widget _stat(String title,String value)=>Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(title,style:TextStyle(color:XyTheme.of(context).muted,fontSize:11)),const SizedBox(height:5),Text(value,style:const TextStyle(fontWeight:FontWeight.w800,fontSize:16))]);
}
